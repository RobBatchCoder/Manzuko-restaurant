...
of converting website viewers into restaurant customers, a large importance was placed on appealing imagery
and appropriate styling. 
The website presents multiple opportunities for customer engagement with reservation
and comment tables in place. The necessary information such as location, menus, restaurant
description and links to social media are provided. However, the customer isn't flooded with information, instead
attractive and enticing imagery is showcased drawing the customer in and encouraging further exploration.

# User Experience
## Primary Goal
The goal of the Manzuko website is to create a pleasant and engaging site that demonstrates the strong aesthetic nature
of the restaurant and products it produces. 

## Visitor Goals
Visitors to Manzuko tend to have an age of 25+ as the restaurant offers a high quality experience and customers pay a premium
for this. These users expect a professional, easy to use and intuitive user experience. Navigation, buttons and layout perform 
...
responsively as users will likely use a variety of devices when choosing a restaurant.

## First time visitor
* Users can easily access all pages through navigation bar.
* Menu items are immediately showcased in the carousel, encouraging further exploration. 
...
* Returning customers can easily access social media links in order to engage with the community
...
## 1. Strategy
* The website needs to be easy to interact with and navigate through as the intended users are not necessarily tech savvy.
* The website must immediately and consistently showcase attract menu items and related imagery in order to draw the user in
and create desire.
* The website needs to have a reservation system in order to convert web users to restaurant attendees.
...
* The website utilises the bootstrap framework to create a mobile first responsive site.
* A brief description of the restaurant is included on the homepage.
* The restaurant is in London, so the website is designed mainly for London residents and tourists.
...
* The footer is consistent across all pages and it will include vital information such as, opening times, address and social media 
links. 
* The navigation bar and the restaurant description section will include a reservation button which will open a reservation model.

## 4. Skeleton
The site was designed to be simply, elegant, and aesthetically pleasing. 
* The logo is displayed at the top left of the view window on every page and acts as a link back to the homepage.
* The nav bar is fixed to the top of the page and allows easy and consistent navigation across the whole webpage. The current page
is underlined on the nav bar so users are always aware of their location on the page.
* The first thing a user sees upon loading the homepage is a large carousel that showcases the appealing menu items from the restaurant.
* For returning visitors who wish to leave a comment about their experience, the contact link on the nav bar takes them straight 
to a form where they can input their personal information and a comment.
* The restaurant description section consists of 2 divs displaying background images on large screen sizes, on mobile view one of
these divs is removed to improve the balance and readability of this section.
* All links displayed on the page open in a new window in the browsers. This prevents users from being taken away from the site. 
and improves usability.
* The gallery page main section contains 9 high quality images. These images exist within the bootstrap column structure and adjust their position and size responsive to the device being used.

## 5. Surface
The top-level design remains consistent across all pages. Borders, margins, and colours follow a consistent theme which allows all.
elements to appear as they belong together.
* The colour Coral is used throughout the project as it represents the seafood element and remains subtle meeting the requirements 
...
* An importance is placed on the aesthetics of the site so the selection of appropriate images was a top priority. High quality, colourful and contrasting images are used throughout.  
...
subtle nature matched with the desired direction for the website. The palette generator in https://coolors.co helped to adds colours to 
the palette that contrasted with coral but wouldn't clash. White was also used through the site in order to create place between the many images used.
![Palette image](assets/readme-files/coolers-palette.png)

## Logo
The logo for this project was created on https://www.wix.com/logo/maker. The primary colour chosen for the website was coral and this contrast nicely with eggshell, so these were the colours used for the logo design. The design was intended to be minimal to high light the subtle minimalist style of the design. Simply the Japanese text 満足 Manzuko, was chosen. This translates to satisfaction in English which worked well for the design.
...
## Navigation Bar
The navigation bar is fully responsive, it provides links to all 4 pages of the website. Included is also a reservation
...
## Reservation booking
In the nav bar and within the restaurant description section is a button which opens a model with the necessary input for a customer to book a table.
...
prompt and anchor link to the full pdf menus which open in a separate window.
...
## Contact form
The contact page includes a contact form which allows the customer to leave a comment and provide the restaurant with
their email and phone number. 
![Contact form image](assets/readme-files/contact-form.png)

## Footer section
The foot section displayed on all pages provides essential information to the customer. This includes opening times, location and links to social media. These links open in a separate page.
![Footer section image](assets/readme-files/footer.png)

# Responsive design
## Desktop, tablet and mobile differences
The bootstrap framework was used to created a responsive website site across mobile, tablet and desktop devices. 
...
* Gallery sections uses Bootstrap grid system to display columns 3x3.
...
## Mobile view
* The nav bar is reduced to a burger menu to save space.
* The restaurant description section is reduced to 1 background div instead of one and the card containing text is centered.
* Gallery images take up the whole width of the view window.

# Future features
* The website in its current state is only a front-end so further development would require fleshing out the backend
in order to make it a functional site.
* Additional pages should be added to highlight events and special occasions hosted at the restaurant.
* In order to keep up menu/decor changes the gallery images should be updated regularly.
...
## 2. Accessibility
* Accessibility score was poor. Aria labels were missing from several elements and some aria labels weren't
unique.
...
* Search engine optimization scored poorly. 
